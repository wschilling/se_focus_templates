...
//todo import lejos.nxt.Motor;
//todo import lejos.nxt.SensorPort;
//todo import lejos.nxt.TouchSensor;
...
public class Robot {
	 ...
	 */
	private CompassDataSource compass;

	private RangefinderDataSource rangefinder;
	...
	public Robot(String host) throws Exception {
		...
		try {
			rangefinder = new RangefinderDataSource(host, "S2", 10.0f);
			rangefinder.addObserver(mc);
		} catch (Exception ex) {
			rangefinder = null;
			ex.printStackTrace();
		}
		compass = new CompassDataSource(host, "S1", 4.0f);
		bsrc = new BatterySource(host);

		ct = new Thread(compass);
		bst = new Thread(bsrc);
		mct = new Thread(mc);
		if (rangefinder != null) {
			rft = new Thread(rangefinder);
		}
	}
